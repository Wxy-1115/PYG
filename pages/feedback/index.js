 ...
 */
  Page({
      ...
      if (chooseImgs.length != 0) {
        chooseImgs.forEach((v, i) => {
          wx.uploadFile({
            ...
            header:	{"token": wx.getStorageSync("token2")},
            // 被上传的文件的路径
            filePath: v,
            // 上传的文件的名称 后台来获取文件  image
            name: "image",
            // 顺带的文本信息
            formData: {},
            success: (result) => {
    ...
    async onLoad() {
      // 获取图床token
      const res = await request2({url: '/token',method: 'POST',data:{"email":"1454866166@qq.com","password":"524335wxy"}})
      console.log(res);
      if(res.code === 200){
        wx.setStorageSync("token2", res.data.token);
      }
    }
  })